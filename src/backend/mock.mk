...
%.green: $(OBJFILES) $(MOCK_OBJS) %_test.o
	$(CC) $(CFLAGS) $(LDFLAGS) -L/usr/local/lib/ $(call WRAP_FUNCS, $(top_srcdir)/$(subdir)/test/$*_test.c) $(call BACKEND_OBJS, $(top_srcdir)/$(subdir)/$*.o) $(filter-out %/objfiles.txt, $^) $(MOCK_LIBS) -lcgreen -o $@
